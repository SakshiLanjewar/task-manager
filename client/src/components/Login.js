...
// ── localStorage — page refresh ke baad bhi data rahe ────────
const STORAGE_KEY = "taskflow_users";
const loadUsers  = () => { try { const d = localStorage.getItem(STORAGE_KEY); return d ? JSON.parse(d) : []; } catch { return []; } };
const saveUsers  = (u) => { try { localStorage.setItem(STORAGE_KEY, JSON.stringify(u)); } catch {} };

// ── EmailJS — real email OTP ──────────────────────────────────
// Apni keys .env file mein daalo:
...
const sendEmailOtp = async (toEmail, otp) => {
  ...
  if (!sid || !tid || !key || sid === "YOUR_SERVICE_ID") {
    alert(`📩 Demo OTP for ${toEmail}:\n\n${otp}\n\n(Add EmailJS keys in .env to send real emails)`);
    return true;
  }
  try {
    const res = await fetch("https://api.emailjs.com/api/v1.0/email/send", {
      ...
      body: JSON.stringify({
        service_id: sid, template_id: tid, user_id: key,
...
// REACT_APP_FAST2SMS_KEY .env mein daalo
const sendSmsOtp = async (phone, otp) => {
  const key = process.env.REACT_APP_FAST2SMS_KEY;
  if (!key || key === "YOUR_FAST2SMS_API_KEY") {
    alert(`📱 Demo OTP for +91${phone}:\n\n${otp}\n\n(Add Fast2SMS key in .env to send real SMS)`);
    return true;
  }
  try {
    const res = await fetch(
      `https://www.fast2sms.com/dev/bulkV2?authorization=${key}&route=otp&variables_values=${otp}&flash=0&numbers=${phone}`,
...
function Login({ onLogin }) {
  ...
  const switchScreen = (s) => { setAnimate(false); setTimeout(() => { setScreen(s); setAnimate(true); }, 180); };
  ...
  const validateReg = () => {
    ...
    if (!regForm.email.trim()) e.email = "Email is required";
      ...
      e.email = "Email already registered. Please sign in.";
    if (!regForm.phone.trim()) e.phone = "Mobile number is required";
    else if (!isPhone(regForm.phone)) e.phone = "Enter valid 10-digit mobile number";
  ...
  const handleRegister = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      const newUser = { name: regForm.name.trim(), email: regForm.email.trim().toLowerCase(), phone: regForm.phone.trim(), password: regForm.password };
      updateUsers([...users, newUser]);
      setLoading(false);
      setSuccessMsg(`🎉 Account created successfully!\n\nWelcome, ${newUser.name}!\n\nYou can now sign in anytime with your email and password.`);
  ...
  const handleLogin = (e) => {
    ...
    setLoading(true);
    setTimeout(() => {
      const user = users.find((u) => u.email === loginForm.email.trim().toLowerCase() && u.password === loginForm.password);
      setLoading(false);
      if (user) {
        onLogin();
      } else if (!users.find((u) => u.email === loginForm.email.trim().toLowerCase())) {
        setLoginErrors({ general: "⚠️ No account found. Please register first." });
      } else {
  ...
  const handleForgotSubmit = async (e) => {
    ...
    if (forgotType === "email") {
      ...
      if (!found) { setForgotError("No account found with this email"); return; }
    } else {
      if (!isPhone(forgotInput)) { setForgotError("Enter valid 10-digit mobile number"); return; }
    ...
    const sent = forgotType === "email" ? await sendEmailOtp(found.email, otp) : await sendSmsOtp(found.phone, otp);
    setLoading(false);
  ...
  const handleOtpVerify = (e) => {
    e.preventDefault();
    if (otpExpired)                  { setOtpError("OTP expired. Please resend."); return; }
    if (!enteredOtp.trim())          { setOtpError("Please enter the OTP"); return; }
    if (enteredOtp !== generatedOtp) { setOtpError("Incorrect OTP. Try again."); return; }
    switchScreen("reset");
  };

  const handleResendOtp = async () => {
    const otp = makeOtp();
    setGeneratedOtp(otp); setEnteredOtp(""); setOtpError(""); setOtpExpired(false);
    ...
    setScreen("otp"); setTimeout(() => setAnimate(true), 50);
  };
  ...
  const handleReset = (e) => {
    e.preventDefault();
    setNewPassError("");
    if (!newPass)                   { setNewPassError("Password is required"); return; }
    if (newPass.length < 6)         { setNewPassError("Minimum 6 characters"); return; }
    if (newPass !== newPassConfirm) { setNewPassError("Passwords do not match"); return; }
    updateUsers(users.map((u) => u.email === forgotTarget ? { ...u, password: newPass } : u));
  ...
  return (
    <div className="login-root">
      ...
      <div className={`login-card ${animate ? "card-visible" : ""}`}>
        ...
        {screen === "login" && (
          <>
            <div className="screen-badge login-badge">👋 Welcome Back</div>
        ...
        {screen === "forgot" && (
          <>
            ...
            <p className="screen-info">Enter your registered Gmail or mobile number. OTP will be sent to verify your identity.</p>
        ...
        {screen === "otp" && (
          <>
            ...
            <p className="screen-info">
              A 6-digit OTP has been sent to your {forgotType === "email" ? "Gmail inbox" : "mobile number via SMS"}.
              {forgotType === "email" ? " Check spam folder too." : ""}
            </p>
        ...
        {screen === "reset" && (
          <>
            ...
            <p className="screen-info">Create a strong new password. Use it to sign in anytime after this.</p>
            <form onSubmit={handleReset} noValidate className="login-form">
              <FInput id="np1" icon="🔒" label="New Password (min 6 characters)"
                type={showNewPass ? "text" : "password"} value={newPass} filled={!!newPass}
                showToggle onToggle={() => setShowNewPass((p) => !p)} showPass={showNewPass}
                onChange={(v) => { setNewPass(v); setNewPassError(""); }}/>
              <FInput id="np2" icon="🔑" label="Confirm New Password"
                type={showNewPassConfirm ? "text" : "password"} value={newPassConfirm} filled={!!newPassConfirm}
                showToggle onToggle={() => setShowNewPassConfirm((p) => !p)} showPass={showNewPassConfirm}
